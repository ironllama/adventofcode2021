...
fn main() {
    ...
    fn process_two(one: usize, two: usize, input_stuff_vec: &mut Vec<Vec<(i32, i32, i32)>>, new_beacons: &mut Vec<(i32, i32, i32)>, scanner_locations: &mut Vec<(i32, i32, i32, i32)>) -> usize {
        ...
        for or_idx in 0..24 {  // 24 orientations of the beacon list.
            ...
            // let mut new_matches: Vec<(i32, i32, i32)> = compare_beacons(&one_vec, &mut two_vec, &mut scanner_two_offset);  // Display's sort requires mut.
    ...
    // Looping through in order does not work, because scanners are not discovered in order. For example, scanner 1 and 4 are matched, and then 2 and 3, then 3 and 4.
    ...
    //                     input_stuff_vec[two] = two_vec.clone();
