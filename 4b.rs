...
fn main() {
    ...
    struct Board {
        ...
        winning_idx: i32,
        winning_num: i32,
        winning_row: Vec<String>,
        winning_type: i32,
        winning_board: Vec<Vec<String>>,
    }

    let mut winning_order: Vec<Board> = vec![];
    for (board_idx, board) in boards.iter().enumerate() {
        ...
        let this_board = Board {
            board_idx: board_idx as i32,
            winning_idx,
            winning_num,
            winning_row: winning_row.iter().map(|x| x.to_string()).collect(),
            winning_type,
            winning_board: winning_board.iter().map(|y| y.iter().map(|x| x.to_string()).collect()).collect(),
        };
    ...
    println!("SCORE: {}", total_uncalled * board_won.winning_num);
}